...
void PmergeMe::init_container1(std::vector<unsigned int> &vec) {
	int size = vec.size();
...
void PmergeMe::init_container2(std::vector<unsigned int> &vec) {
	int size = vec.size();
	for (int i = 0; i < size; ++i) {
...
void PmergeMe::sort_container1() {
    /* Split the array */
    std::vector<std::vector<unsigned int> > splitArrays = split_into_k<std::vector<unsigned int> >(container1, 2);
    ...
    /* Merge the arrays */
    std::vector<unsigned int> merged = splitArrays[0];
...
void PmergeMe::sort_container2() {
    /* Split the array */
    std::vector<std::deque<unsigned int> > splitArrays = split_into_k<std::deque<unsigned int> >(container2, 2);
    ...
    /* Merge the arrays */
    std::deque<unsigned int> merged = splitArrays[0];
    for (size_t i = 1; i < splitArrays.size(); i++) {
...
void PmergeMe::print_and_check_sorted() {
    std::vector<unsigned int>::iterator it1 = container1.begin();
    std::deque<unsigned int>::iterator it2 = container2.begin();
